...
// calls RNG and compares result to fixed probability
...
// Takes in prob (1/s) and deltaT (s) and returns the probability that
// the event occurs at least once (single event)
class CondFixedSingleProb : public Cond 
{
  public:
    explicit CondFixedSingleProb(double prob) : m_prob(prob) 
      {assert(prob>=0);assert(prob<=1);};
    // ~CondFixedProb();			// use default destructor

    bool test(const vector<double> &values, double deltaT) const
      {return ( RandK::randk() < 1-pow(1-m_prob, deltaT) );};

  private:
    double m_prob;

    // not used
    CondFixedSingleProb(const CondFixedSingleProb &c);
    //    CondFixedSingleProb operator = (const CondFixedSingleProb &c);
};

// Takes in prob (1/s) and deltaT (s) and returns the expected
// number of times the event occurs
/**************************************************
 * NOT RECOMMENDED: WILL TOP OUT AT p*dt >= 1 !!! *
 * Use a rate if this is likely to happen.        *
 *************************************************/
class CondFixedExpectedProb : public Cond 
{
  public:
    explicit CondFixedExpectedProb(double prob) : m_prob(prob) 
	{assert(prob>=0);assert(prob<=1);};
    // ~CondFixedProb();			// use default destructor
  ...
  private:
    ...
    // not used
    CondFixedExpectedProb(const CondFixedExpectedProb &c);
    //    CondFixedExpectedProb operator = (const CondFixedExpectedProb &c);
};
...
// calls Rate to calculate base probability, calls RNG on result*deltaT
class CondCalcProbSingle : public Cond 
{
  public:
    explicit CondCalcProbSingle(Rate *pr) : m_pr(pr) {};
    // ~CondCalcProb();			// use default destructor
    ...
    bool test(const vector<double> &values, double deltaT) const
    {
      	...
      	return false;
       	...
       	return true;
	 	    ...
	 	    return ( RandK::randk() < 1-pow(1-prob,deltaT) );
		};
  ...
  private:
    ...
    // not used
    CondCalcProbSingle(const CondCalcProbSingle &c);
    CondCalcProbSingle operator = (const CondCalcProbSingle &c);
};

// calls Rate to calculate base probability, calls RNG on result*deltaT
class CondCalcProbExpected : public Cond 
{
  public:
    explicit CondCalcProbExpected(Rate *pr) : m_pr(pr) {};
    // ~CondCalcProb();			// use default destructor
  ...
  private:
    ...
    // not used
    CondCalcProbExpected(const CondCalcProbExpected &c);
    CondCalcProbExpected operator = (const CondCalcProbExpected &c);
};
#endif
