...
struct DecayUntil {
  ...
  DecayUntil(double _rate, int _time) {
    rate = _rate;
    time = _time;
  }
};
...
class Molecule {
  public:
    ...
    void addDecayUntil(double rate, int time);
    ...
    void removeMols(double amount, const SimPoint &p);
    ...
    Conc getTotalConc() const;
    double getTotalMols() const;
    ...
    double getInterpNumMolecules(double volume, const SimPoint &pos) const;
  ...
  private:
    ...
    vector<DecayUntil> m_decayUntil;
    unsigned int m_decayUntilCounter;
    int m_time;
