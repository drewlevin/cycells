...
class Cells {
  public:
    ...
    void addHexMixIndex(const string type1, const string type2, 
		    double perc, double zpos);
    ...
    Cell* addCell(const string& typeName, SimPoint pos, bool birth = 1);	
    Cell* addCell(int typeIndex, SimPoint pos, bool birth = 1);
    ...
    int getXRange() const {return m_xrange;};
    int getYRange() const {return m_yrange;};
    int getZRange() const {return m_zrange;};

    // ---------------------- OUTPUT ROUTINES -----------------------------
    void writeDefinition(ofstream &outfile);
    void writeData(ofstream &outfile);
    void writeIndex(char* file);
    void writeFuzzy(double center_x, double center_y);

    int fuzz_count;
    double fuzzy[50];
    double inside[50];
    double perimeter[50];
    double cell_count[50];
  ...
  private:
    ...
    void bounceBC(SimPoint &pos, SimPoint& vel, SimPoint& dir);
