...
Molecule::Molecule(const string& title) : m_name(title), 
	m_diffusionRate(0), m_decayRate(0), m_decayUntilCounter(0),
  m_time(0)
{
  initialize();
}
...
Molecule::Molecule(const string& title, double diff, double decay) 
    ...
    m_decayUntilCounter(0), m_time(0)
{
  assert(diff>=0);
  initialize();
}
 ...
 * Returns - nothing                                                    *
 ************************************************************************/
void Molecule::addDecayUntil(double rate, int time)
{
  DecayUntil newDecayUntil(rate, time);
  m_decayUntil.push_back(newDecayUntil);
}
...
void Molecule::changeConc(double amount, const SimPoint &p)
{
  ...
  m_concentration.at(xi, yi, zi) += change;
	if (m_concentration.at(xi, yi, zi) < 0)
		m_concentration.at(xi, yi, zi) = 0;
...
// molecules rather than adding them - stick to simpler scheme for nowu
...
/************************************************************************ 
 * removeMols()                                                         *
 *   Lowers concentration in the area surrounding the given point.      *
 *                                                                      *
 * Parameters                                                           *
 *   int amount:   	        #of molecules to subtract                   * 
 *   const SimPoint &p:         grid location                       	  *
 *                                                                      *
 * Returns - nothing                                                    *
 ************************************************************************/
void Molecule::removeMols(double amount, const SimPoint &p)
{
  // Ensure that we are not trying to remove more molecules than exist.
  assert(getTotalMols() > amount);
  ...
  // amount passed in should be #molecules:
  // convert to moles/ml based on volume of grid cell and subtract from
  // conc want amount/(N_AV*volume) - denominator precalculated in 
  // setGeometry and inverted
  Conc total = amount * sm_invNavVol;
  Conc temp = m_concentration.at( xi, yi, zi );
  
  if (temp >= total)
    m_concentration.at(xi, yi, zi) -= total;
  else {
    total -= temp;
    m_concentration.at(xi, yi, zi) = 0;

    // 2D Version
    for (int r=1; r<sm_xsize/2; r++)
    {
      // Direction
      for (int d=r; d<r+4; d++)
      {
        ...
        for (int n=0; n<r; n++)
        {
          ...
          // Wrap Around
          if (xn <= 0) xn += sm_xsize;
          else if (xn >= sm_xsize+1) xn -= sm_xsize;
          if (yn <= 0) yn += sm_xsize;
          else if (yn >= sm_xsize+1) yn -= sm_xsize;
          
          // Get local concentration
          temp = m_concentration.at(xn, yn, zi);

          if (temp >= total)
          {
            m_concentration.at(xn, yn, zi) -= total;
            return;
          }
          else
          {
            total -= temp;
            m_concentration.at(xn, yn, zi) = 0;
          }
        }
      }
    }
...
{ 
  ...
  if (m_decayUntilCounter < m_decayUntil.size())
    decay_factor = pow(1-m_decayUntil[m_decayUntilCounter].rate, deltaT);
  else
    decay_factor = pow(1-m_decayRate, deltaT);
  ...
  for (i=0; i<=sm_xsize+1; i++)
    for (j=0; j<=sm_ysize+1; j++)
      ...
      {
        m_concentration.at(i,j,k)=decay_factor*m_concentration.at(i,j,k);
...
//  double diff_factor = (1-pow(1-m_diffusionRate, deltaT))/pow(sm_gridsq,2);
    ...
    for (j=1; j<=sm_ysize; j++)
      {
        ...
        m_deltaConc.at(i,j,1) = -(1-decay_factor)*current;
...
{ 
  ...
  double diff_factor = m_diffusionRate * deltaT / pow(sm_gridsq,2);
  ...
  for (i=1; i<=sm_xsize; i++)
    for (j=1; j<=sm_ysize; j++)
      for (k=1; k<=sm_zsize; k++)
      {
        ...
        m_deltaConc.at(i,j,k) = -(1-decay_factor)*current;
        sum = 0;
...
void Molecule::update(double deltaT)
{
  ...
  if (m_decayUntilCounter < m_decayUntil.size())
  {
    if (m_time > m_decayUntil[m_decayUntilCounter].time)
      m_decayUntilCounter++;
  }
...
/************************************************************************ 
 * getTotalConc()                                                       *
 *   Calculates total concentration over all indices                    *
 *                                                                      *
 * Parameters - none                                                    *
 *                                                                      *
 * Returns - average concentration for tissue (moles/ml)                *
 ************************************************************************/
Molecule::Conc Molecule::getTotalConc() const
{
  Molecule::Conc total=0.0;
...
/************************************************************************ 
 * getTotalMols()                                                       *
 *   Calculates total number of molecules                               *
 *                                                                      *
 * Parameters - none                                                    *
 *                                                                      *
 * Returns - average concentration for tissue (moles/ml)                *
 ************************************************************************/
double Molecule::getTotalMols() const
{
  Molecule::Conc total=0.0;
...
int Molecule::getNumMolecules(double volume, const SimPoint &pos) const
{
  ...
  // multiply by N_AV and volume specified (vol in um^3)
  return int( 6.022E11 * getConc(pos) * volume );
}

/************************************************************************ 
 * getInterpNumMolecules()                                              *
 *   Returns number of molecules within the grid cell containing the    *
 *   specified location                                                 *
 *                                                                      *
 * Parameters -                                                         *
 ...
 * Returns - number of molecules (int)                                  * 
 ************************************************************************/
double Molecule::getInterpNumMolecules(double volume, const SimPoint &pos) const
{
  // determine concentration at specified point;
  // convert moles/ml to number of molecules
  // multiply by N_AV and volume specified (vol in um^3)
  return double( 6.022E11 * getInterpConc(pos) * volume );
}
