...
cout << "constructor seed = " << m_seed << endl;
  RandK::setSeed(-labs(m_seed));	// get random number generator ready
...
void Tissue::update(double deltaT)
{
  ...
  simtime += deltaT;
/*
  // Get culture sizes at 30, 35, and 40h
  if (simtime == 108000 || simtime == 126000 || simtime == 144000)
  {
    char file[256];
    pid_t pid = getpid();
    sprintf(file, "snaps/index%d_%dh.txt", pid, (int)(simtime / 3600.0));
    cells->writeIndex(file);
    if (simtime == 144000)
    {
      cells->writeFuzzy(getXSize()/2, getYSize()/2);
    } 
  }
*/
}
