 ...
 * Returns - nothing               					*
 ************************************************************************/
Cell* Cells::addCell(const string& type_name, SimPoint pos, 
		bool birth /* = 1 */)
{
  ...
  return addCell(index, pos, birth);
}

Cell* Cells::addCell(int index, SimPoint pos, 
	bool birth /* = 1 */)
{
  assert (index >= 0);
 ...
 *   pattern.  Similar to addHexSheet, but this routine creates the 	*
 *   grid with two cell types, using the specified percentages of each. *
 *   This method also sets the index attribute of any type2 cell to a 
 *   unique value if that cell type has an index attribute.							*
 * Parameters          			 				*
 *   string type1, type2	names of cell types we're adding	*
 *   double perc1,       	percentage of type 1 to use		*
 *   double zpos:     		specifies location of plane in 3D grid  *
 ...
 * Returns - nothing               					*
 ************************************************************************/
void Cells::addHexMixIndex(const string type1, const string type2, 
		double perc, double zpos)
{
  assert( (zpos >= 0) && (zpos < m_zrange) );
  ...
  int index = 1;
  int attrIdx = t2->getAttributeIndex("index");
  ...
  for (double x=radius; x<(m_xrange); x+=hspace)
    if (odd)
    {
      for (double y=radius; y<(m_yrange); y+=diameter)
      {
        ...
        {
          pc = addCell(type2, SimPoint(x,y,zpos), false);
          if (attrIdx >= 0)
            pc->setValue(attrIdx, index++);
        } 
      }
      odd = false;
    }
...
/************************************************************************ 
 * checkNeighbors                             				*
 *   Returns a goolean value indicating whether there is a cell of the  *
 ...
 * Returns - nothing               					*
 ************************************************************************/
void Cells::bounceBC(SimPoint& pos, SimPoint& vel, SimPoint& dir)
{
  if (pos.getX() < 0)
  {
    pos.setX(-pos.getX());
...
void Cells::moveCells(double deltaT)
{
  for (unsigned int i=0; i<cell_list.size(); i++)
  {
    ...
    if (pct->getSpeed()) 	// is this a mobile cell?            
    {
      ...
      Vnet += pc->getDirection() * pct->getSpeed();
...
{
  vector<CellType*>::iterator itr;

  outfile << "cell_detail:  " << cell_list.size() << endl;
  for (unsigned int i=0; i<cell_list.size(); i++)
    outfile << *cell_list[i] << endl;

  for (itr = cell_type_list.begin(); itr != cell_type_list.end(); itr++) 
  {
    if ((*itr)->getAttributeIndex("index") != -1)
    {
      writeIndex("snaps/index.txt");
      break;
    }
  }
}
...
/************************************************************************ 
 * writeIndex()                                                         *
 *   creates a new file with a column representing the population of    *
 *   cells that match a given index value                               *
 *									                                                    *
 * Parameters -                                                        	*
 *   ofstream outfile:		where to write data to                        *
 *                                                                      *
 * Returns - nothing                                                    *
 ************************************************************************/
void Cells::writeIndex(char* file)
{
  map<int, unsigned int> count;
  vector<Cell*>::iterator vector_itr;
  map<int, unsigned int>::iterator map_itr;
  int index_value;

  for (vector_itr = cell_list.begin(); vector_itr != cell_list.end(); vector_itr++)
  {
    index_value = (*vector_itr)->getValue(cell_type_list[(*vector_itr)->getTypeIndex()]->getAttributeIndex("index"));
    if (index_value != -1)
    {
      if (count.find(index_value) == count.end())
        count[index_value] = 1;
      else
  ...
  for (map_itr = count.begin(); map_itr != count.end(); map_itr++)
    outfile << map_itr->first << "\t" << map_itr->second << endl;      
}

/************************************************************************ 
 * writeFuzzy()                                                         *
 *   Prints out a plaque's 'fuzzy index', caculated as:                 *
 *									                                                    *
 * Average Radius of cells in plaque / (2/3)*sqrt((6nr^2)/(pi*sqrt(3))) *
 *									                                                    *
 * where n is the number of cells infected/expressing/dead and r is the *
 * radius of such cells.                                                *
 *									                                                    *
 * Returns - nothing                                                    *
 ************************************************************************/
void Cells::writeFuzzy(double center_x, double center_y)
{
  vector<Cell*> cell_neighbors;
  vector<Cell*>::iterator cell_itr, neighbor_itr;
  int inside_count = 0, infected_cells = 0, border_count = 0;
  double min_radius, temp_radius, avg_radius = 0;
  double fuzzy_index, inside_index, perimeter_index;
  double total_perimeter, min_perimeter;
  double r = getCellType(cell_list[0]->getTypeIndex())->getRadius();


  // Fuzzy Index (Avg Distance from Center vs Minimal Avg Distance from Center)
  for (cell_itr = cell_list.begin(); cell_itr != cell_list.end(); cell_itr++)
  {
    if ((*cell_itr)->getTypeIndex() > 0)
    {
      infected_cells++;
      avg_radius += sqrt(pow((*cell_itr)->getPosition().getX() - center_x, 2) + 
                         pow((*cell_itr)->getPosition().getY() - center_y, 2));
    }
  }
  min_radius = sqrt((6.0*infected_cells*r*r) / (M_PI*sqrt(3.0)));
  avg_radius /= infected_cells;
  fuzzy_index = avg_radius / ((2.0/3.0) * min_radius);
/*  cout << "=====================================" << endl;
  cout << "-------------------------------------" << endl;
  cout << "Fuzzy Index: " << fuzzy_index << endl; */

  fuzzy[fuzz_count % 50] = fuzzy_index;

  // Inside Index (Percent of Cells Inside Minimal Circle)
  for (cell_itr = cell_list.begin(); cell_itr != cell_list.end(); cell_itr++)
  {
    if ((*cell_itr)->getTypeIndex() > 0)
    {
      temp_radius = sqrt(pow((*cell_itr)->getPosition().getX() - center_x, 2) + 
                           pow((*cell_itr)->getPosition().getY() - center_y, 2));
      if (temp_radius < min_radius)
        inside_count++;
    }
  }
  inside_index = (double)inside_count / (double)infected_cells;
//  cout << "Inside Index: " << inside_index << endl;
  inside[fuzz_count % 50] = inside_index;

  // Perimeter (Total Perimeter vs Minimal Perimeter)
  for (cell_itr = cell_list.begin(); cell_itr != cell_list.end(); cell_itr++)
  {
    if ((*cell_itr)->getTypeIndex() > 0)
    {
      cell_neighbors.clear();
      getNeighbors(*cell_itr, cell_neighbors);
      for (neighbor_itr = cell_neighbors.begin(); neighbor_itr != cell_neighbors.end(); neighbor_itr++)
      {
        if ((*neighbor_itr)->getPosition().dist((*cell_itr)->getPosition()) <= 2.1*r &&
            (*neighbor_itr)->getTypeIndex() == 0) {
          border_count++;
        }
      }
    }
  }
//  total_perimeter = border_count * (2*r/sqrt(3));
  total_perimeter = border_count * 2 * r / sqrt(3);
  min_perimeter = 2 * M_PI * min_radius;
  perimeter_index = total_perimeter / min_perimeter;
/*  cout << "Perimeter Index: " << perimeter_index << endl;
  cout << "Total Cells: " << infected_cells << endl;
  cout << "-------------------------------------" << endl;
  cout << "=====================================" << endl;  */

  perimeter[fuzz_count % 50] = perimeter_index;

  cell_count[fuzz_count % 50] = infected_cells;
