...
cout << "molecule type name:  " << buff << endl;
  ...
  while (buff[0] != '}')
  {
    ...
    else if (strcmp(buff, "decay_until") == 0)
    {
      infile >> rate;
      infile >> time;
      pm->addDecayUntil(rate, time);
    }
...
cout << "action type:  " << buff << endl;
  ...
  else if (strcmp(buff, "infect") == 0)
  {
    Molecule *pm = readMolName(pt, infile);
    int index = readCellName(pt, infile);
    double radius = pct->getRadius();
    pa = new ActionInfect(pm, index, radius);
  }
  ...
  else if (strcmp(buff, "change_index") == 0)
  {
    // read cell type name, get its index
    int index = readCellName(pt, infile);
    Cells *cells = pt->getCellsPtr();
    pa = new ActionChangeIndex(index, cells);
  }
  else if (strcmp(buff, "divide") == 0)
  {
    ...
    int index = readCellName(pt, infile);
    Cells *cells = pt->getCellsPtr();
    pa = new ActionDivide(cells, index);
  }
  else if (strcmp(buff, "admit") == 0)
  {
    ...
    double dist = 2*pct->getRadius();
    Cells *cells = pt->getCellsPtr();
    pa = new ActionAdmit(index, dist, flag, cells);
  }
  else if (strcmp(buff, "admit2D") == 0)
  {
    int index = readCellName(pt, infile);
    infile >> buff;		// initialization scheme
    bool flag;
    ...
    double dist = 2*pct->getRadius();
    Cells *cells = pt->getCellsPtr();
    pa = new ActionAdmit2D(index, dist, flag, cells);
  }
  ...
  else if (strcmp(buff, "admit_rand") == 0)
  {
    int index = readCellName(pt, infile);
    infile >> buff;		// initialization scheme
    bool flag;
    ...
    else error("FileDef: unknown initialization option for action admit_rand");
    Cells *cells = pt->getCellsPtr();
    Rate *pr = readRate(pt, pct, infile);
    pa = new ActionAdmitRand(index, flag, cells, pr);
  }
  else if (strcmp(buff, "admit_rand2D") == 0)
  {
    int index = readCellName(pt, infile);
    infile >> buff;		// initialization scheme
    bool flag;
    ...
    else error("FileDef: unknown initialization option for action admit_rand2D");
    Cells *cells = pt->getCellsPtr();
    Rate *pr = readRate(pt, pct, infile);
    pa = new ActionAdmitRand2D(index, flag, cells, pr);
  }

  else if (strcmp(buff, "recirculate") == 0)
  {
    int index = readCellName(pt, infile);
    infile >> buff;		// initialization scheme
    bool flag;
    ...
    else error("FileDef: unknown initialization option for action recirculate");
    Cells *cells = pt->getCellsPtr();
    pa = new ActionRecirculate(index, flag, cells);
  }
  else if (strcmp(buff, "recirculate2D") == 0)
  {
    int index = readCellName(pt, infile);
    infile >> buff;		// initialization scheme
    bool flag;
    ...
    else error("FileDef: unknown initialization option for action recirculate2D");
    Cells *cells = pt->getCellsPtr();
    pa = new ActionRecirculate2D(index, flag, cells);
  }
  else if (strcmp(buff, "recirculate2D0") == 0)
  {
    int index = readCellName(pt, infile);
    infile >> buff;		// initialization scheme
    bool flag;
    ...
    else error("FileDef: unknown initialization option for action recirculate2D0");
    Cells *cells = pt->getCellsPtr();
    pa = new ActionRecirculate2D0(index, flag, cells);
  }
  ...
  else if (strcmp(buff, "move_randomly2D") == 0)
  {
	...
	else if (strcmp(buff, "turn_randomly") == 0)
	{
		double std;
		infile >> std;
		pa = new ActionTurnRandomly(std);
	}
  else if (strcmp(buff, "turn_randomly2D") == 0)
  {
		double std;
		infile >> std;
    pa = new ActionTurnRandomly2D(std);
  }
  ...
  else if (strcmp(buff, "move_bias2D") == 0)
  {
    infile >> buff;         // molecule name
    Molecule *pm = pt->getMolecule(buff);
    double min;
    ...
    double bias;
    infile >> bias;
    pa = new ActionBiasMoveChemotaxis2D(pm, min, r, bias);
  }
...
Molecule *FileDef::readMolName(Tissue *pt, ifstream &infile)
{
  ...
  Molecule *pm = pt->getMolecule(buff);
  if (!pm)
    error("FileDef::readMolName - can't find molecule", buff);
  return pm;
}
		...
		ifstream &infile)
{
  ...
  else if (strcmp(buff, "fixed_single") == 0)
  {
    double prob;
    infile >> prob;
    pc = new CondFixedSingleProb(prob);         
  }

  else if (strcmp(buff, "fixed_expected") == 0)
  {
    double prob;
    infile >> prob;
    pc = new CondFixedExpectedProb(prob);         
  }
  ...
  else if (strcmp(buff, "calc_prob_single") == 0)
  {
    Rate *pr = readRate(pt, pct, infile);
    pc = new CondCalcProbSingle(pr);         
  }

  else if (strcmp(buff, "calc_prob_expected") == 0)
  {
    Rate *pr = readRate(pt, pct, infile);
    pc = new CondCalcProbExpected(pr);         
  }
